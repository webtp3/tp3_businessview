...
//
    ...
    /**
     * Add vHost's domain to htaccess as exception rule for general domain rewrite
     *
     * @param $vHost
     *
     */
    function configureVHost($vHost)
    {
        $vHost = str_replace('.', '\.', $vHost);
        $this->taskReplaceInFile('/web/.htaccess')
            ->from('# __ENVIRONMENT_DOMAIN__')
            ->to('# __ENVIRONMENT_DOMAIN__
RewriteCond %{HTTP_HOST} \!\^' . $vHost . '$ [NC]')
    ...
    /**
     * Build dependencies (npm etc.) and add .htaccess rewrite
     *
     */
    function configureProjectForContext($context = null)
    {
        if (empty($context)) {
            $context = $this->context;
        }

        if (strtolower($context) != 'production') {
    ...
    /**
     * Build project configuration (conf.d)
     */
    function buildProjectConfiguration()
    {
    ...
    function buildAssets($context = null)
    {
        ...
        $this->say('[TASK] Build assets for context <bg=yellow;fg=black>' . strtoupper($context) . '</bg=yellow;fg=black> ...');
        $this->say('[TASK] Build assets for refactored pages...');
        ...
        $this->say('[TASK] Build assets for legacy pages...');
        $this->taskExec('compass compile --force')
            ->dir(self::BASE_DIR . '/web')
            ->run();
    }
     ...
     *
     * Loads content of http://www.myproject.com/ to generate critical css
     *
     * TODO: find a way to add a list of URLs to build critical CSS for
     *
     */
    function buildCritical() {
        $this->say('[TASK] Build critical css...');
    ...
    function optimizeImages() {
        $this->say('[TASK] Optimize images (' . self::BASE_DIR . '/web/assets/Images)...');
    ...
    function optimizeSvgIcons() {
        $this->say('[TASK] Optimize svg icons (reduce svg overhead) in ' . self::BASE_DIR . '/web/assets/Images ...');
     ...
     *      ./vendor/consolidation/robo/robo test:functional http://www.myproject.com 1
     *
     * For jenkins (with selenium grid) start with:
     *      ./vendor/consolidation/robo/robo test:functional http://www.myproject.com 0
     *              OR
     *      ./vendor/consolidation/robo/robo test:functional http://www.myproject.com
     *
     * @param string $domain vHost to test against
     * @param bool $local If local is set to true wdio starts standalone selenium server
     */
    function testFunctional($domain = 'http://www.myproject.com', $local = false) {
    ...
    function testAcceptance($seleniumPath = self::BASE_DIR.'/build/vendor/se/selenium-server-standalone/bin/selenium-server-standalone.jar')
    {
     ...
     *
     * TODO: think about a more dynamic apprach, eg. sassing all scss files in Scss root dir instead of hardcoding them
     *
     */
