...
//        $I->see(getenv('HOST_TITLE'));
        ...
        $I->click('Cal Indexer');
        $I->amOnPage('/kontakt.html');
        $I->click('tx_powermail_pi1[field][datenschutz][]');
        $I->type(tx_powermail_pi1[field][nachricht], '$this->faker->word');
        $I->click('Senden');
        $I->waitForElement('.powermail_confirmation', 30);
        $I->see('Sind diese Werte richtig?');
        $I->click('Weiter');
        $I->waitForElement('#c1678', 30);
        $I->see('Danke für Ihre Anfrage');
