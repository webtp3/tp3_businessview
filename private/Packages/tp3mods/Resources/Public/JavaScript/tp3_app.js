...
var scroll_pos = $j(document).scrollTop(),
    ...
    logoheight = headerheight * 0.9,
    logowidth = $j('.navbar-brand-image').width(),
    ...
    if(mobile != true || headerwidth > 600) {
        ...
        if(scroll_pos  == (headerPos)) {
            ...
            show = setTimeout(function() {
                ...
                $j(' a.navbar-brand-image, #logo, .logo').height(logoheight).css({width:"auto"});
                $j('.navbar-collapse:not(.in) .nav > li > a, .headerslogan').css({"line-height": (headerheight - toolbarheight) * 0.4  -0.5 +"px"});
        ...
        if(scrollPos > headerPos && scroll_pos <= scrollPos) {
            ...
            $j('header.navbar-top').height(headerheight).css({position:"relative"})
            $j(' a.navbar-brand-image, #logo, .logo').width(logowidth).height(logoheight);
            $j('.navbar-collapse .nav > li > a, .headerslogan').css({"line-height": (headerheight - toolbarheight) * 0.4  -0.5 +"px"});
...
//$j('.main-section .frame').css({"min-height":screen.height});
