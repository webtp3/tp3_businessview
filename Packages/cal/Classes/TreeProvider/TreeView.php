...
class TreeView
{
     ...
     */
    public function displayCalendar($PA, &$fobj): string
    {
        $table = $PA['table'];
        $field = $PA['field'];
        ...
        if (is_array($cfgArr) && is_array($cfgArr['data']['s_Cat']['lDEF']) && is_array($cfgArr['data']['s_Cat']['lDEF']['calendarSelection'])) {
            $selectedCalendars = GeneralUtility::trimExplode(
                ',',
                $cfgArr['data']['s_Cat']['lDEF']['calendarSelection']['vDEF'],
                1
            );
        }
        ...
        if ($confArr['useStoragePid']) {
            ...
            $storagePid = $TSconfig['_STORAGE_PID'] ?: 0;
        ...
        if (!$GLOBALS['BE_USER']->user['admin'] && $field === 'pi_flexform') {
            ...
            if ($GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']) {
                ...
                if ($GLOBALS['BE_USER']->user['tx_cal_calendar']) {
                    $be_userCalendars = array_merge(
                        $be_userCalendars,
                        GeneralUtility::trimExplode(',', $GLOBALS['BE_USER']->user['tx_cal_calendar'], 1)
                    );
                }
            }
        ...
        if ($calres) {
            while ($calrow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($calres)) {
                if ((TYPO3_MODE === 'BE') || ($GLOBALS['TSFE']->beUserLogin && $GLOBALS['BE_USER']->extAdmEnabled)) {
                    $tempRow = BackendUtility::getRecordLocalization(
                        'tx_cal_calendar',
                        $calrow['uid'],
        ...
        $test->requiredElements[$PA['itemFormElName']] = [
            $minitems,
            $maxitems,
     ...
     * @return string HTML code for the field
     */
    public function displayCategoryTree($PA, $fobj): string
    {
        $table = $PA['table'];
        $field = $PA['field'];
        $row = $PA['row'];
        ...
        if ($field === 'pi_flexform') {
            ...
            if (is_array($cfgArr) && is_array($cfgArr['data']['s_Cat']['lDEF']) && is_array($cfgArr['data']['s_Cat']['lDEF']['calendarSelection'])) {
                $selectedCalendars = GeneralUtility::intExplode(
                    ',',
                    $cfgArr['data']['s_Cat']['lDEF']['calendarSelection']['vDEF']
                );
            }
            ...
            if ($row['sys_language_uid'] && $row['l18n_parent']) { // the current record is a translation of another record
                ...
                if ($table === 'tx_cal_event') {
                    $catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                        ...
                        'tx_cal_category,tx_cal_event_category_mm',
                        'tx_cal_category.uid = tx_cal_event_category_mm.uid_foreign and tx_cal_event_category_mm.uid_local=' . $row['l18n_parent'] . $SPaddWhere
                    );
                } else {
                    $catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                        'tx_cal_category.uid, tx_cal_category.title',
                        'tx_cal_category',
                        'tx_cal_category.uid=' . $row['l18n_parent'] . $SPaddWhere
                    );
                }
                $categories = [];
                $NACats = [];
                ...
                $minitems = MathUtility::forceIntegerInRange($config['minitems'], 0);
                // Register the required number of elements:
                $this->pObj->requiredElements[$PA['itemFormElName']] = [
                    $minitems,
                    $maxitems,
                    'imgName' => $table . '_' . $row['uid'] . '_' . $field
                ];
                ...
                if ($config['treeView'] and $config['foreign_table']) {
                    ...
                    $treeOrderBy = $confArr['treeOrderBy'] ?: 'uid';
                    ...
                    if ($table === 'be_users' || $table === 'be_groups') {
                                    ...
                                    foreach ($GLOBALS['BE_USER']->userGroups as $gid => $group) {
                                        if ($group['tx_cal_enable_accesscontroll']) {
                                            if ($group['tx_cal_category']) {
                                                $groupCategories = GeneralUtility::trimExplode(
                                                    ',',
                                                    $group['tx_cal_category'],
                                                    1
                                                );
                                                $be_userCategories = array_merge($be_userCategories, $groupCategories);
                                            }
                                            if ($group['tx_cal_calendar']) {
                                                $groupCalendars = GeneralUtility::trimExplode(
                                                    ',',
                                                    $group['tx_cal_calendar'],
                                                    1
                                                );
                        ...
                        } elseif ($isPluginFlexform && $GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll'] && !$GLOBALS['BE_USER']->user['admin']) {
                            $allowAllCalendars = false;
                            $allowAllCategories = false;
                            $be_userCategories = [
                                0
                        ...
                        } elseif ($isPluginFlexform && count($selectedCalendars) > 0) {
                            if ($calendarMode == 0) {
                            } elseif ($calendarMode == 1) {
                                $catWhere = ' AND tx_cal_category.calendar_id in (' . implode(
                                    ',',
                                    $selectedCalendars
                                ) . ')';
                    ...
                    $catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                        'tx_cal_category.uid, tx_cal_category.title, tx_cal_category.calendar_id, tx_cal_category.parent_category',
                        'tx_cal_category',
                        '1=1' . $catWhere,
                        $treeOrderBy
                    );
                    ...
                    $categoryById = [];
                    $categoryByCalendarId = [];
                    $categoryByParentId = [];
                    $allCategoryById = [];
                    $allCategoryByCalendarId = [];
     ...
     * @return array
     */
    public function checkChildIds($uid, &$categoryByParentId): array
    {
     ...
     */
    public function getNotAllowedItems($PA, $SPaddWhere): array
    {
     ...
     */
    public function findRecursiveCategories($PA, $row, $table, $storagePid, $treeIds): array
    {
        $errorMsg = [];
        if ($table === 'tt_content' && $row['CType'] === 'list' && $row['list_type'] == 9) { // = tt_content element which inserts plugin tt_news
            ...
            if (is_array($cfgArr) && is_array($cfgArr['data']['sDEF']['lDEF']) && $cfgArr['data']['sDEF']['lDEF']['categorySelection']) {
                $rcList = $this->compareCategoryVals(
                    $treeIds,
     ...
     */
    public function compareCategoryVals($treeIds, $catString): string
    {
        $recursiveCategories = [];
     ...
     */
    public function displayTypeFieldCheckCategories(&$PA, $fobj): string
    {
        $table = $PA['table'];
        $field = $PA['field'];
        $row = $PA['row'];
        ...
        if ($GLOBALS['BE_USER']->getTSConfigVal('options.useListOfAllowedItems') && !$GLOBALS['BE_USER']->isAdmin()) {
            ...
            if ($notAllowedItems[0]) {
                ...
                $catres = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
                    ...
                    'tt_news_cat_mm',
                    'tt_news_cat',
                    ' AND tt_news_cat_mm.uid_local=' . $uidField . $SPaddWhere,
                    '',
                    'mmsorting'
                );
                $NACats = [];
