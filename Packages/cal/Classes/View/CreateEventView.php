...
class CreateEventView extends \TYPO3\CMS\Cal\View\FeEditingBaseView
{
    ...
    public function getDescriptionMarker(& $template, & $sims, & $rems)
    {
        ...
        if ($this->isAllowed('description')) {
            ...
            if (is_object($this->RTEObj) && $this->RTEObj->isAvailable() && $this->conf ['rights.'] [$this->isEditMode ? 'edit.' : 'create.'] ['event.'] ['enableRTE']) {
                ...
                $RTEItem = $this->RTEObj->drawRTE($this, 'tx_cal_event', $this->strEntryField, $row = [], $this->PA, $this->specConf, $this->thisConfig, $this->RTEtypeVal, '', $this->thePidValue);
    ...
    public function getFormStartMarker(& $template, & $sims, & $rems, & $wrapped)
    {
        ...
        $rems ['###FORM_START###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($temp, $temp_sims, [], []);
    }
            ...
            while ($row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc($result)) {
                ...
                if ($this->conf ['view.'] [$this->conf ['view'] . '.'] ['freeAndBusyViewPid'] && $row ['tx_cal_calendar'] && $this->rightsObj->isLoggedIn()) {
                    ...
                    if ($freeAndBusyIsEnabled && ! empty($calendarOwner)) {
                        ...
                        $this->controller->pi_linkTP('test', [
