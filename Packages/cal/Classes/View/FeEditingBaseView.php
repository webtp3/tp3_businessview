...
class FeEditingBaseView extends \TYPO3\CMS\Cal\View\BaseView
{
    ...
    public function getFormEndMarker(& $template, & $sims, & $rems, & $wrapped)
    {
        ...
        $rems ['###FORM_END###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($temp, $temp_sims, [], []);
    }
        ...
        if ($this->isConfirm) {
                ...
                foreach ($_FILES [$this->prefixId] ['name'] [$marker] as $id => $filename) {
                    ...
                    if ($_FILES [$this->prefixId] ['error'] [$marker] [$id]) {
                        ...
                        if (in_array($fI ['fileext'], $denyExt)) {
                            continue;
                        } elseif ($marker == 'image' && ! in_array($fI ['fileext'], $allowedExt)) {
                            continue;
                        }
                    ...
                    $sims ['###' . strtoupper($marker) . '###'] .= \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($temp, $temp_sims, [], []);
            ...
            if ($this->isEditMode && $this->rightsObj->isAllowedTo('edit', $this->objectString, $marker)) {
                ...
                for (; $i < $max; $i ++) {
                    ...
                    $temp_sims ['###INDEX###'] = $i;
                    $upload = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($upload, $temp_sims, [], []);
                }
                ...
                for ($i = 0; $i < $max; $i ++) {
                    ...
                    $temp_sims ['###INDEX###'] = $i;
                    $sims ['###' . strtoupper($marker) . '###'] .= \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached($upload, $temp_sims, [], []);
                }
            }
        }
    }
            ...
            if (! empty($langIds)) {
                ...
                foreach ($langIds as $key => $langId) {
                    if ($langId > 0) {
                        ...
                        $sims ['###TRANSLATION_OPTIONS###'] .= ' ' . $this->controller->pi_linkTP($this->cObj->cObjGetSingle($this->conf ['view.'] ['translation.'] ['languageMenu.'] [$langId], $this->conf ['view.'] ['translation.'] ['languageMenu.'] [$langId . '.']), [
