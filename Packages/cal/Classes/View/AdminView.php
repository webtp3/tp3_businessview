    ...
    private function checkAction()
    {
        switch ($this->controller->piVars ['adminAction']) {
            case 'editCalendarSubscription':
                ...
                $ids = is_array($this->controller->piVars ['calendarSubscription']) ? $this->controller->piVars ['calendarSubscription'] : ($this->controller->piVars ['calendarSubscription'] != '' ? \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->controller->piVars ['calendarSubscription'], 1) : []);
                $allIds = $this->controller->piVars ['calendarSubscriptionIds'] ? \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->controller->piVars ['calendarSubscriptionIds'], 1) : [];
                $fields = [
