...
abstract class BaseService extends \TYPO3\CMS\Core\Service\AbstractService
{
    ...
    protected static function insertIdsIntoTableWithMMRelation($mm_table, $idArray, $uid, $tablename, $additionalParams = [], $switchUidLocalForeign = false)
    {
        $uid_local = 'uid_local';
                ...
                foreach ($_FILES [$this->prefixId] ['name'] [$type] as $id => $filename) {
                    if ($_FILES [$this->prefixId] ['error'] [$type] [$id]) {
                        ...
                        if (in_array($fI ['fileext'], $denyExt)) {
                            continue;
                        } elseif ($type == 'image' && ! empty($allowedExt) && ! in_array($fI ['fileext'], $allowedExt)) {
                            continue;
                        }
