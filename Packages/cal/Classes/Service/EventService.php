...
class EventService extends \TYPO3\CMS\Cal\Service\BaseService
{
     ...
     */
    public function getEventsFromTable(&$categories, $includeRecurring = false, $additionalWhere = '', $serviceKey = '', $addCategoryWhere = false, $onlyMeetingsWithoutStatus = false, $eventType = '0,1,2,3')
    {
        $calendarService = &$this->modelObj->getServiceObjByKey('cal_calendar_model', 'calendar', 'tx_cal_calendar');
        $categoryService = &$this->modelObj->getServiceObjByKey('cal_category_model', 'category', $this->extConf ['categoryService']);
     ...
     * @return array array of events represented by the model.
     */
    public function findAll($pidList, $eventType = '0,1,2,3')
    {
        // How to get the events
        // 1st get Calendar specified
        // 2nd get categories specified
     ...
     */
    public function find($uid, $pidList, $showHiddenEvents = false, $showDeletedEvents = false, $getAllInstances = false, $disableCalendarSearchString = false, $disableCategorySearchString = false, $eventType = '0,1,2,3')
    {
        $uid = intval($uid);
        if ($getAllInstances) {
    ...
    public function _saveEvent(&$eventData, $object)
    {
        ...
        } elseif ($this->rightsObj->isLoggedIn() && $this->conf ['rights.'] ['create.'] ['event.'] ['addFeUserToNotify']) {
            $this->insertIdsIntoTableWithMMRelation('tx_cal_fe_user_event_monitor_mm', [
        ...
        if ($this->conf ['rights.'] ['create.'] ['event.'] ['public']) {
            $this->insertIdsIntoTableWithMMRelation('tx_cal_fe_user_event_monitor_mm', GeneralUtility::trimExplode(',', $this->conf ['rights.'] ['create.'] ['event.'] ['notifyUsersOnPublicCreate'], 1), $uid, 'fe_users', [
                    'offset' => $this->conf ['view.'] ['event.'] ['remind.'] ['time'],
                    'pid' => $eventData ['pid']
            ]);
        }
        ...
        if ($this->rightsObj->isAllowedToCreateEventCategory()) {
            ...
            $this->insertIdsIntoTableWithMMRelation($category_mm_relation_table, [
    ...
    public function search($pidList = '', $start_date, $end_date, $searchword, $locationIds = '', $organizerIds = '', $eventType = '0,1,2,3')
    {
     ...
     */
    public function getDaysOfWeekShort()
    {
        return [
                'Sun',
                'Mon',
    ...
    public function saveExceptionEvent($pid)
    {
        ...
        $this->insertIdsIntoTableWithMMRelation('tx_cal_exception_event_mm', [
                $uid
        ...
        for ($i = 0; $i < count($byDayArray); $i ++) {
            ...
            if (preg_match('/([-\+]{0,1})?([0-9]{1})?([A-Z]{2})/', $byDayArray [$i], $byDaySplit)) {
                if (! in_array($byDaySplit [3], $allowedWeekdayValues)) {
                    continue;
                } elseif (! ($byDaySplit [2] > 0 && ($event->getFreq() == 'month' || $event->getFreq() == 'year'))) {
    ...
    public function findDailyWithin(&$master_array, $event, $startRange, $endRange, $weekdays, $maxCount, &$currentCount, &$totalCount, &$addedCount, &$maxRecurringEvents)
    {
    ...
    public function getMonthDaysAccordingly(&$event, $month, $year)
    {
        $byDayArray = $event->getByDay();
